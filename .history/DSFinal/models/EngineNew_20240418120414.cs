...
public class DatabaseEngine<T> : IEnumerable<Item<T>> where T : class
{
    ...
    private Dictionary<string, IIndex<Guid, T>> indices;

    public DatabaseEngine()
    {
        table = new Dictionary<Guid, Item<T>>();
        indices = new Dictionary<string, IIndex<Guid, T>>();
    }
    ...
    {
        IIndex<Guid, T> index = new UniqueIndex<TKey, Item<T>>(keySelector);
    ...
    public override string ToString()
    {
        ...
        foreach (var indexEntry in indices)
        {
            foreach (var item in indexEntry)
            {
