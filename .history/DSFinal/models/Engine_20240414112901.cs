...
public class Engine<TValue> where TValue : class
{
    private Table<TValue> table;
    private UniqueIndex<Guid, TValue> uniqueIndex;
    private NonUniqueIndex<string, TValue> nonUniqueIndex; // Example index by a string property, e.g., Name
    private RangeIndex<DateTime, TValue> rangeIndex; // Example for a range index, assuming a DateTime property

    public Engine()
    {
        table = new Table<TValue>();
        uniqueIndex = new UniqueIndex<Guid, TValue>();
        nonUniqueIndex = new NonUniqueIndex<string, TValue>();
        rangeIndex = new RangeIndex<DateTime, TValue>();
    }
