...
public class DatabaseEngine<T> where T : class
{
    ...
    public void AddRecord(Guid id)
    {
        UpdateIndicesAdd(table.Get(id));
        table.Add(id, table.Get(id));
    }

    public void RemoveRecord(Guid id)
    {
        UpdateIndicesRemove(table.Get(id));
        table.Remove(id);
    }

    public void UpdateRecord(Guid id)
    {
        UpdateIndicesUpdate(table.Get(id));
        table.Update(id, table.Get(id));
    }
    ...
    private void UpdateIndicesUpdate(T record)
    {
        foreach (var indexEntry in indices)
        {
            dynamic index = indexEntry.Value;
